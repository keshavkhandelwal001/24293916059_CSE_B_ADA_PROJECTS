...
void print_matrix(int n, int **M, const char *name) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            printf("%4d ", M[i][j]);  // 4 spaces wide
        }
        printf("\n");
    }
}

void multiply_dc(int n, int **A, int **B, int **C) {
        if (n <= 2) {  
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++) {
                C[i][j] = 0;
    ...
    // Split matrix A and B into submatrices
    ...
    multiply_dc(k, A11, B11, T1);
    multiply_dc(k, A12, B21, T2);
    add_matrix(k, T1, T2, C11);

    
    multiply_dc(k, A11, B12, T1);
    multiply_dc(k, A12, B22, T2);
    add_matrix(k, T1, T2, C12);


    multiply_dc(k, A21, B11, T1);
    multiply_dc(k, A22, B21, T2);
    add_matrix(k, T1, T2, C21);

    
    multiply_dc(k, A21, B12, T1);
    multiply_dc(k, A22, B22, T2);
    add_matrix(k, T1, T2, C22);
...
int main() {
    int sizes[] = {63, 150, 200, 300}; // not necessarily powers of 2
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        // Fill only the original n×n part with random numbers
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++) {
                A[i][j] = rand() % 10;
                B[i][j] = rand() % 10;
        ...
        clock_t start = clock();
        multiply_dc(m, A, B, C);   // work on padded matrices
        clock_t end = clock();

        double time_taken = ((double)(end - start)) / CLOCKS_PER_SEC;
        printf("Execution time (Divide & Conquer): %.6f seconds\n", time_taken);

        // Print only original n×n result
        // print_matrix(n, C, "Result");
